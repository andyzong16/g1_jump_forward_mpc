...
def main(headless=False, steps=500, scene="flat", vx=0.3, vy=0.0, wz=0.0):
    ...
    def step_controller():
        ...
        tau_fb = 10.0 * (q_ref - qpos[7 : 7 + config.n_joints]) - 2.0 * (
            qvel[6 : 6 + config.n_joints]
        )
        data.ctrl = np.asarray(tau + tau_fb)
